...
class Room():
    ...
    def __init__(self,
                 ...
                 room_temp_inside: int = 24):

        self.room_systemname = room_systemname
        # self.add_id()
        # self.add_room_systemname_item(self)
        # Room.items.append(self)
        # self.id = self.id[-1]
        self.room_name: str = room_name
        self.room_level: str = room_level
        self.room_area_m2: float = room_area_m2
        self.room_high_m: float = room_high_m
        self.room_volume_m3: float = self.room_area_m2 * self.room_high_m
        self.Fw: float = round(6 * pow(self.room_volume_m3, 0.667), 2)
        self.opening1_area: float = 0
        self.A0: float = 0
        self.room_fire_load_density: float = room_fire_load_density  # Плотность пожарной нагрузки помещения
        self.Fw_unit_fire_load_by_walling: float = 0
        self.room_calorific_value_fire_load = room_calorific_value_fire_load  # Средняя теплота сгорания пожарной нагрузки
        self.calorific_value_fire_load_mass = []
        self.v0_air_for_burn: float = 0  # V0 Удельное количество воздуха, необходимое для полного сгораняи пожарной нагрузки
        self.room_opening_rate: float = 0  # Проемность помещения
        self.unit_fire_load_critical = 0  # gkkr Удельное критическое количество пожарной нагрузки
        self.unit_fire_load_by_floor_square = 0  # g0 Удельная приведенная пожарная нагрузка, отнесенная к площади пола помещения
        self.fire_type = Fire_type.FIRE_BY_VENT  # Вид объемного пожара
        self.room_temp_inside = room_temp_inside  # Начальная температура воздуха в помещении
        self.temp_inside_K: int = 273 + self.room_temp_inside
        self.max_temp: int = 0  # Максимальная среднеобъемная температура в помещении K
        self.temp_smoke_coridor: int = 0  # Максимальная среднеобъемная температура в коридоре K
        self.corridor_hight = 0
        self.corridor_door_hight = 0
        self.corridor_door_width = 0
        self.corridor_area = 0
        self.corridor_lenght = 0
        self.corridor_smoke_hight_limit = 0  # Предельная толщина дымового слоя
        self.corridor_temp = 0
        self.corridor_temp_K = 0
    ...
    def __str__(self):
        return f"class Room: id-{self.id}, system - {self.room_systemname}, room_name - {self.room_name}"
    ...
    #
    # def add_room_systemname_item(cls, self) -> None:
    #     if self.room_systemname not in cls.room_systemname_items:
    #         cls.room_systemname_items[self.room_systemname] = [self]
    #     else:
    #         cls.room_systemname_items[self.room_systemname].append(self)
    #
    # @classmethod
    # def get_rooms_by_system_and_name(cls, system:str, room_names:list):
    #     rooms = cls.room_systemname_items[system]
    #     rooms_result = []
    #     for room in rooms:
    ...
    #     return rooms_result
    #
    # def get_area_m2(self, room_area_m2)-> None:
    #     self.room_area_m2 = room_area_m2
    #
    # def get_high_m(self, room_high_m)-> None:
    #     self.room_high_m = room_high_m
    #
    # def calc_room_volume_m3(self)-> None:
    #     self.room_volume_m3 = self.room_area_m2 * self.room_high_m
    ...
    def get_opening1_area(self, opening1_area) -> None:
        self.opening1_area = opening1_area
    ...
    def calc_A0(self) -> None:
        ...
        """
        self.A0 = self.opening1_area

    # def get_fire_load_density(self, room_fire_load_density: int)-> None:
    #     """
    ...
    def calc_unit_fire_load_by_floor_square(self) -> None:
        ...
        Приложение 1 МР 2013
        """
        self.unit_fire_load_by_floor_square = self.room_fire_load_density / CALORIFIC_VALUE_WOOD
    ...
    def define_type_of_fire(self) -> None:
        ...
        FIRE_BY_FIRELOAD = 2
        """
        self.fire_type.FIRE_BY_VENT if \
            self.Fw_unit_fire_load_by_walling > self.unit_fire_load_critical \
            else self.fire_type.FIRE_BY_FIRELOAD

    # def get_temp_inside(self, room_temp_inside : int) -> None:
    #     self.room_temp_inside = room_temp_inside

    # def get_temp_coridor(self, temp_smoke_coridor: int) -> None:
    #     self.temp_smoke_coridor = temp_smoke_coridor
    #     self.corridor_temp_K = temp_smoke_coridor + 273
